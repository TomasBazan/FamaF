...
void fat_fuse_log_activity(char *operation_type, fat_file file,
  ...
  // printf("%s\n\n", buf);
  // ver como se calcula el offset
  int result = fat_file_pwrite(fs_log, buf, message_size, offset, fs_parent);
  if (result < 0) {
    EXIT_FAILURE;
  }
}
...
int fat_fuse_write(const char *path, const char *buf, size_t size, off_t offset,
  ...
  // llamo a log_activity
  fat_fuse_log_activity("Write", file, vol);
...
int fat_fuse_unlink(const char *path) {
  ...
  // Update parents directory and save changes on disk
