...
static int fill_dentry_time_now(fat_dir_entry dir_entry, bool fill_create,
  ...
  if (!(!fill_create || fill_modified)) return -1;
  if (fill_modified) {
...
void write_dir_entry(fat_file parent, fat_file file) {
  ...
  DEBUG("Writting dentry on directory %s, entry %u the file's name is %s",
        parent->name, file->pos_in_parent, file->name);
  ...
  for (disk_dentry_ptr = (fat_dir_entry)buffer; disk_dentry_ptr <= end_ptr;
    ...
    if (ignore_dentry(disk_dentry_ptr)) {
      ...
      *free_entry_index = dir_entries_processed;
      dir->dir.free_entries =
          g_list_append(dir->dir.free_entries, free_entry_index);
      continue;
    }
...
ssize_t fat_file_pread(fat_file file, void *buf, size_t size, off_t offset,
  ...
  while (bytes_remaining > 0) {  // There are still bytes to read
    ...
    bytes_read =
        full_pread(file->table->fd, buf, bytes_to_read_cluster, cluster_off);
...
ssize_t fat_file_pwrite(fat_file file, const void *buf, size_t size,
  ...
  if (offset % fat_table_bytes_per_cluster(file->table) == 0 && cluster != 0 &&
      offset != 0) {
