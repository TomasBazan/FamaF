...
float average(list l)
{
    list_elem testing;
    float sumatoria = 0;
    float length = list_length(l);
    if (length != 0)
    {
        for (unsigned int i = 1; i < length + 1; i++)
        {
            testing = list_index(l, i);
            sumatoria = sumatoria + testing;
        }
        sumatoria = sumatoria / length;
    }
...
list array_to_list(int array[], unsigned int length)
{
    ...
    nuevaLista = list_empty();

    for (unsigned int i = 0u; i < length; ++i)
    {
        nuevaLista = list_addr(nuevaLista, array[i]);
    }
    /* Return list */
    return nuevaLista;
}
